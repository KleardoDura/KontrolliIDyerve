...
public class Hyrja extends JFrame implements ActionListener {
	...
	Hyrja(){
		...
		hyre =new JButton("Hyre");
		hyre.setBounds(152,100,70,30);
		hyre.setFocusable(false);
		hyre.addActionListener(this);
		this.add(hyre);
			...
			try {
				...
				if(DbConnection.kontrolloDeren(dera)!=0) {
					...
					if(DbConnection.kontrolloPunonjesin(dera,punonjesi)!=0 ) {
						...
						//Update daten e hyrjes se fundit
							DbConnection.updateDatenEHyrjes(punonjesi);
						JOptionPane.showMessageDialog(null,"Pershendetje!Dera eshte e hapur!");
						}	
						else {
